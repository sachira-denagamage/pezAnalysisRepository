...
for i = 1:length(expIDlist)
    ...
    try
    trkdir = fullfile('Z:\hhmiData\dm11\cardlab\Data_pez3000_analyzed\',exptID,'APT_Results\'); %directory where trk files should go
...
%     fullset.Video_Path = replace(fullset.Video_Path,'/dm11','groups/card');
    ...
    if vidind>0
        ...
        for  j=1:length(vidind)
            fullset.Video_Path{j} = fullfile('Z:\hhmiData\dm11\Data_pez3000', fullset.VideoName{j}(16:23), fullset.VideoName{j}(1:23),[fullset.VideoName{j} '.mp4']);
            fullset.Video_Path = replace(fullset.Video_Path,'\','/');
        end
    end
